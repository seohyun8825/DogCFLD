...
from datasets import HappyDataset
...
class build_model(nn.Module):
    def __init__(self, cfg):
        ...
        pose_guidance_channels = cfg.MODEL.POSE_GUIDANCE_CONFIG.CHANNELS
        ...
        self.pose_encoder = PoseEncoder(
            ...
            channels=pose_guidance_channels,
            groups=cfg.MODEL.POSE_GUIDANCE_CONFIG.GROUPS
        )
        ...
        self.decoder = Decoder(
            ...
            pose_channel=pose_guidance_channels[-1]
        )
...
def main(cfg):
    ...
    pretrained_path = "/home/user/Desktop/CFLD_pl/CFLD/pretrained_models/vae"

    vae = VariationalAutoencoder(pretrained_path).to(accelerator.device, dtype=weight_dtype)

    scheduler_directory = "/home/user/Desktop/CFLD_pl/CFLD/pretrained_models/scheduler"
    ...
    if cfg.MODEL.PRETRAINED_PATH:
        ...
        checkpoint_dir = "/home/user/Desktop/CFLD_pl/CFLD/outputs/CFLD/debug/checkpoint/checkpoints"
    ...
    for epoch in range(last_epoch, cfg.OPTIMIZER.EPOCHS, 1):
        ...
        for i, batch in enumerate(train_loader):
            # 디버깅: 배치 데이터 확인
            logger.info(f"Batch {i}: {batch.keys()}")
            logger.info(f"img_src shape: {batch['img_src'].shape}")
            logger.info(f"pose_img_src shape: {batch['pose_img_src'].shape}")
            logger.info(f"img_tgt shape: {batch['img_tgt'].shape}")
            logger.info(f"pose_img_tgt shape: {batch['pose_img_tgt'].shape}")
        ...
        if (epoch + 1) % 40 == 0:  
            ...
            checkpoint_file = os.path.join(save_dir, "latest_checkpoint")
            if os.path.exists(checkpoint_file):
                shutil.rmtree(checkpoint_file)
            accelerator.save_state(checkpoint_file)
